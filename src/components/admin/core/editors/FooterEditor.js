...
function FooterEditor({ components: C }) {
  ...
  return (
    <C.Panel>
      ...
      <C.PanelContent>
        <C.FormGroup><C.Label>Tagline</C.Label><C.Input value={content.tagline || ''} onChange={(e) => update('tagline', e.target.value)} placeholder="Wir freuen uns auf euch!" /></C.FormGroup>
        <C.FormGroup><C.Label>Hashtag</C.Label><C.Input value={content.hashtag || ''} onChange={(e) => update('hashtag', e.target.value)} placeholder="#PauliUndMo2026" /></C.FormGroup>
