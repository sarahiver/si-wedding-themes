...
export default function AddressSearch({ components: C, address, onSelect, label, placeholder }) {
  ...
  const styles = {
    ...
    icon: {
      position: 'absolute', left: '0.75rem', color: 'var(--admin-text-subtle, rgba(255,255,255,0.3))',
      pointerEvents: 'none', display: 'flex', alignItems: 'center', zIndex: 1,
    },
    input: {
      width: '100%', padding: '0.6rem 0.8rem 0.6rem 2.25rem',
      background: apiAvailable ? 'var(--admin-bg-subtle, rgba(255,255,255,0.05))' : 'var(--admin-bg-subtle, rgba(255,255,255,0.02))',
      border: '1px dashed var(--admin-border, rgba(255,255,255,0.15))',
      borderRadius: '4px', color: 'var(--admin-text, rgba(255,255,255,0.85))', fontSize: '0.9rem',
      fontFamily: 'inherit', outline: 'none', transition: 'border-color 0.2s',
      opacity: apiAvailable ? 1 : 0.5,
      cursor: apiAvailable ? 'text' : 'not-allowed',
    },
    loading: {
      position: 'absolute', right: '0.75rem', color: 'var(--admin-text-subtle, rgba(255,255,255,0.3))',
      display: 'flex', alignItems: 'center',
      animation: 'spin 1s linear infinite',
    },
    dropdown: {
      position: 'absolute', top: 'calc(100% + 4px)', left: 0, right: 0,
      background: 'var(--admin-bg-subtle, #1a1a1a)', border: '1px solid var(--admin-border, rgba(255,255,255,0.15))',
    ...
    item: (active) => ({
      ...
      background: active ? 'var(--admin-bg-hover, rgba(255,255,255,0.08))' : 'transparent',
      borderBottom: '1px solid var(--admin-border-subtle, rgba(255,255,255,0.06))',
    }),
    itemIcon: { color: 'var(--admin-text-subtle, rgba(255,255,255,0.3))', flexShrink: 0, display: 'flex' },
    mainText: { fontSize: '0.85rem', fontWeight: 500, color: 'var(--admin-text, rgba(255,255,255,0.85))' },
    subText: { fontSize: '0.7rem', color: 'var(--admin-text-muted, rgba(255,255,255,0.4))', whiteSpace: 'nowrap', overflow: 'hidden', textOverflow: 'ellipsis' },
    hint: {
