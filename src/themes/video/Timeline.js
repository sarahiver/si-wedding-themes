...
const Item = styled.div`display: grid; grid-template-columns: 80px 1fr; gap: 1.5rem; align-items: center; text-align: left; opacity: 0; animation: ${p => p.$visible ? css`${fadeUp} 0.8s var(--ease-out-expo) forwards` : 'none'}; animation-delay: ${p => 0.2 + p.$index * 0.1}s;`;
const Time = styled.span`font-family: var(--font-display); font-size: 1.25rem; font-weight: 600; color: var(--video-accent);`;
const ItemContent = styled.div`padding: 1rem 0; border-bottom: 1px solid rgba(255,255,255,0.1);`;
