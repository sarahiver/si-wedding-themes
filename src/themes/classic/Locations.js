...
function Locations(){const{content}=useWedding();const ld=content?.locations||{};const locs=ld.locations?.length?ld.locations:[{label:'Trauung & Feier',name:'Schloss Charlottenburg',description:'Die Zeremonie und Feier finden im prachtvollen Ambiente des Schlosses statt.',address:'Spandauer Damm 20, 14059 Berlin',image:'https://res.cloudinary.com/si-weddings/image/upload/v1770720374/siwedding/demo-botanical/hotels/jzbjzmnwiisapct8yevz.jpg',accent_image:'https://res.cloudinary.com/si-weddings/image/upload/v1770723589/siwedding/demo-botanical/gallery/cxlyozhisgrvupguqsa6.jpg'}];
return(<>{locs.map((loc,i)=>{const[ref,v]=useInView(0.08);const rev=i%2===1;return(<Sec key={i} ref={ref} $rev={rev} id={i===0?'locations':undefined}><ImgSide><img src={loc.image||'https://res.cloudinary.com/si-weddings/image/upload/v1770720374/siwedding/demo-botanical/hotels/jzbjzmnwiisapct8yevz.jpg'} alt={loc.name} loading="lazy"/>{loc.accent_image&&<AccentImg $rev={rev}><img src={loc.accent_image} alt="" loading="lazy"/></AccentImg>}</ImgSide><TxtSide>{loc.label&&<Eye $v={v}>{loc.label}</Eye>}<Title $v={v}>{loc.name||loc.titel}</Title><P $v={v}>{loc.description||loc.beschreibung}</P>{(loc.address||loc.adresse)&&<Addr $v={v}>{loc.address||loc.adresse}</Addr>}</TxtSide></Sec>);})}</>);}
