...
return(<Page><Card><Eye>{s.tagline||'Save the Date'}</Eye><Names>{n1}<ScriptAnd>&</ScriptAnd>{n2}</Names><Dt>{fmt(weddingDate)}</Dt>{loc&&<Loc>{loc}</Loc>}<Msg>{s.message||'Einladung folgt'}</Msg></Card></Page>);}
